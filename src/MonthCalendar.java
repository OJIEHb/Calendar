...
public class MonthCalendar {
    ...
    public void printCalendar() {
        ...
        for (int i = 1; i <= month.lengthOfMonth(); i++) {
            ...
            DayOfWeek monthDayName = monthDay.getDayOfWeek();
            if (monthDay.equals(LocalDate.now())) {
                System.out.print(CURRENT_DAY_COl + dayValue);
            } else if (monthDayName.equals(DayOfWeek.SATURDAY) || monthDayName.equals(DayOfWeek.SUNDAY)) {
                System.out.print(DAY_OFF_COl + dayValue);
            } else {
                System.out.print(COMMON_DAY_COl + dayValue);
            }
            if (monthDayName.equals(DayOfWeek.SUNDAY)) {
                System.out.println();
            }
        }
    }
